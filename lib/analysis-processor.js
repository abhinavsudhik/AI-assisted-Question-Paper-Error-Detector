...
export async function runAnalysisJob({
    ...
    try {
        ...
        if (fileData && fileData.size > 15 * 1024 * 1024) {
            throw new Error('File too large (max 15MB). Please compress or split the document and try again.');
        }
        ...
        if (!geminiCooldownUntil) {
            ...
            try {
                ...
                const timeoutPromise = new Promise((_, reject) => {
                    timeoutId = setTimeout(() => reject(new Error('timeout')), PROVIDER_TIMEOUT_MS);
                });
                ...
                if (geminiError.message !== 'timeout') {
                    ...
                    console.warn('[Provider] Gemini timed out — falling through to next provider');
                }
        ...
        if (!responseText && !groqCooldownUntil) {
            try {
                ...
                const isTimeoutErr = groqError.name === 'AbortError' || 
                                     groqError.message?.toLowerCase().includes('timeout') || 
                                     groqError.message?.toLowerCase().includes('aborted');
                ...
                if (!isTimeoutErr) {
                    ...
                    console.warn('[Provider] Groq timed out — falling through to next provider');
                }
                // Always fall through to next provider — never throw from a provider catch block
            }
        ...
        if (!responseText) {
            try {
                ...
                console.error('[Provider] OpenRouter failed:', openRouterError.message);
        ...
        console.error(`[process-job] Error for analysis ${analysisId}:`, error?.message || error);

        const errMsg = error?.message || 'An unknown error occurred during analysis.';
        ...
        try {
            ...
            console.error(`[process-job] Critical: Failed to update database status to failed for ${analysisId}:`, dbError.message);
        }
...
async function callGroqLlama(prompt) {
    ...
    try {
        ...
        const timeoutPromise = new Promise((_, reject) => {
            fetchTimeoutId = setTimeout(() => reject(new Error('timeout')), PROVIDER_TIMEOUT_MS);
        });
